        ...
        schema: z.object({
          ...
          ingredients: z.array(
            z
              .string()
              .describe(
                `Each ingredient listing. Include the name and the amount and description as written in the recipe. Specify if they're optional`
              )
          ),
        }),
      },
      ...
      await Promise.all(
        parsed.ingredients.map((ingredient) => {
          const ingredientInsertQuery = {
            ...
            text: `INSERT INTO recipe_ingredients (id, text, recipe_id)
          VALUES ($1, $2, $3);`,
            values: [randomUUID(), ingredient, body.id],
          }
